...
The first is between 28 March 2008 and 25 January 2009 (302 Earth days\change{/19 Titan days}) and 26 November 2010 and 9 September 2011 (286 Earth days\change{/18 Titan days}).

\begin{figure}[ht!]
\plotone{Fig/IMG_interval}
\caption{\change{Timeline of the ISS/NAC CL1-UV3 images analyzed (vertical black ticks). The number of observations per 2 Earth months period (\ie 4 Titan days / $\Delta Ls \approx \ang{2}$) is reported with a green gradient scale. The gray area correspond to the gaps with no available observations.}}
...
\change{This deconvolution method is known to be efficient to restore fine strutures in astronomical images \citep{Hunt1996}.}
...
\change{On average, the image pixel scale is about $10 \pm 5$~km.}
...
In the detached haze layer, the multiple scattering is mainly produced by the light coming from the \change{main haze} below. To
evaluate $\varrho_k$, \change{we use a representative vertical profile of the main haze that}
...
1.05 and \change{1.20}, depending on the geometry of the observation.
\change{This effect is included in our model. We also checked that significant changes in the main haze (single scattering albedo, the opacity and the vertical scale height) only affect the value of $\varrho_k$ by a few percent and can be neglected.}
...
$\left< \varpi P(\Theta)\right>_k$ is the average of the product of the single scattering and the phase function at the scattering angle of the observation $\Theta$ for the layer crossed on $x_k$.  $\beta_k(z)$ is the local extinction \change{coefficient} at altitude $z$ \change{(and the product of the haze cross-section and the local abunance)}. Here, the altitude $z(x)$ is the local altitude at point of abscissa $x$ along the line of sight.
...
Moreover, the tangential opacity along a line of sight ($\tau_{los}$) is considered opaque when it reaches 3 \change{(usually around 300 km in the UV)}.
...
\change{The uncertainties associated to the intensity profile ($I/F$) correspond to a $1 \sigma$ distribution of the observed $I/F$ for a given altitude within a \ang{5} bin in latitude.
...
The uncertainties in the extinction profile ($\beta$) correspond to the minimum and maximum values required to fit the data within $1 \sigma$ of the observed $I/F$ in each layer independently \citep[contrary to][ where the errors are computed in all the layers simultaneously]{West2018}.}
...
on the resampled data (orange error bars) \change{within a \ang{5} bin in latitude}.
